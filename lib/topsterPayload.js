...
export function encodeTopsterData({ links, theme = 'grid', w = DEFAULT_GRID.w, h = DEFAULT_GRID.h }) {
    const normalizedTheme = theme === 'classic' ? 'classic' : 'grid';
    const safeW = clampInt(w, 1, 10, DEFAULT_GRID.w);
    const safeH = clampInt(h, 1, 10, DEFAULT_GRID.h);
    const total = getLayoutSize({ theme: normalizedTheme, w: safeW, h: safeH });
    const layout = normalizedTheme === 'classic' ? 'c' : `g${safeW}x${safeH}`;
    const source = Array.isArray(links) ? links : [];

    const tokens = source.slice(0, total).map((link) => {
        if (!link || !VIDEO_ID_PATTERN.test(String(link.id || ''))) return '';
        const seconds = Math.max(0, parseInt(link.t, 10) || 0);
        return seconds ? `${link.id}~${seconds.toString(36)}` : String(link.id);
    });

    while (tokens.length && tokens[tokens.length - 1] === '') tokens.pop();
    return `v1:${layout}:${tokens.join('.')}`;
}
